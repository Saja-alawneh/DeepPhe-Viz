      ...
      for(j=0;j<patientGroups[i].data.length; j++)// loop through the documents for each patients
        {
          ...
          if (!(month in CountsPerYearAndQuarter[year]))
          {
            ...
            for (var f=0; f < fields.length; f++) 
            {
                ...
                var report = patientGroups[i].data[j].documentType;
                if (!(report in totals[year][month])) 
                {
                    totals[year][month][report]=0;
		            
                }
                else 
                {
                    totals[year][month][report] = parseInt(totals[year][month][report])+ 1
                }                    
               // console.log("*******************")
...
let labelData={};
let docTypeCounts=[]
for (var key in totals ) // loop for the year
{
    for (var key1 in totals[key]) // loop for the months
    {
        docTypeCounts.push({'month':key1, 'reportRad':totals[key][key1]['Radiology Report'],
        'reportPath': totals[key][key1]['Surgical Pathology Report'],
        'reportDis': totals[key][key1]['Discharge Summary'],
        'reportClin' : totals[key][key1]['Clinical Note'],
        'reportProg':totals[key][key1]['Progress Note'] })
    }
}
...
for (var key in CountsPerYearAndQuarter ) 
{
    ...
    for (var key1 in CountsPerYearAndQuarter[key]) 
    {
       ...
       //iterate through the Labels 
        ...
        {  
        transformed.push({'year': key, 'month': parseInt(key1), 'field': key2, 
            'value':parseInt(CountsPerYearAndQuarter[key][key1][key2]['counts']),
                'patienList': CountsPerYearAndQuarter[key][key1][key2]['patients']})
...
let docTypeC=[]
for(var i=0;i<docTypeCounts.length; i++)
    {
    var month=docTypeCounts[i].month

    if(typeof docTypeCounts[i].reportRad !== 'undefined') 
    {
   var Rad= docTypeCounts[i].reportRad
    }

    if(typeof docTypeCounts[i].reportPath !== 'undefined') 
    {
    var Path= docTypeCounts[i].reportPath
    }

    if(typeof docTypeCounts[i].reportDis !== 'undefined') 
    {
    var Dis=docTypeCounts[i].reportDis
    }

    if(typeof docTypeCounts[i].reportProg !== 'undefined') 
    {
    var Prog=docTypeCounts[i].reportProg
    }

    if(typeof docTypeCounts[i].reportClin !== 'undefined') 
    {
    var clinical=docTypeCounts[i].reportClin
    }
    docTypeC.push({month, Rad, Path, Dis, Prog, clinical})
}


// To calculate the Label-month prevlance 
/*var output1 = _(transformed)
            .groupBy(x => x.month)
            .map((objs, key ) => 
                ({'month': key, 'LabelmontheTotal': _.sumBy(objs, 'value')}))
            .value();
...
while (i<output1.length)
{
    ...
    {
        if (output1[i].month == transformed[j].month)
        {
        
        transformed[j].value1=transformed[j].value /output1[i].LabelmontheTotal * 100
        }
    }
...
const output1 = [];
patientGroups.forEach(function(entry){
    entry.data.forEach(r => {
        output1[r["Month"]] = (output1[r["Month"]] || 0) + 1;
})
})
...
{
   for(j=0; j<transformed.length; j++)
   {
       if (key == transformed[j].month)
       {
           //console.log(key, transformed[j].month)
           //console.log(key,transformed[j].month,transformed[j].value, output1[key])
           transformed[j].value1=Math.ceil((transformed[j].value /output1[key]))
       }
   }
}
        ...
        for (i=w; i<episodeSortedTar.length;i++)
            {
                if (episodeSortedTar[i].source==source && episodeSortedTar[i].target==target)
                    {
                        patientListEpisode.push(episodeSortedTar[i].patient)
                        cnt =cnt +1
                    }
                else 
                    {
...
for(var j=0; j< finalEpisode.length; j++)
{
   ...
   /* Links1.push({"source": nodeWithoutDup.indexOf(S), 
                    "target": nodeWithoutDup.indexOf(T),
                    "value": finalEpisode[j].value,
                    "patienList":finalEpisode[j].patienList})*/
    Links.push({"source": S, 
                    "target":T,
                    "value": finalEpisode[j].value,
                    "patienList":finalEpisode[j].patienList})
