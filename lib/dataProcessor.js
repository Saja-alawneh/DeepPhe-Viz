...
class DataProcessor {
    ...
    static getDiagnosis(patientIds, neo4jRawArr) {
        ...
        console.log(neo4jRawArr.length);
        epsiode=[]
...
//patientdata= neo4jRawArr //item[Object.keys(rawdata)[0]]
      ...
      for(j=0;j<patientGroups[i].data.length; j++)// loop through the documents for each patients
        {
            ...
            for (f=0; f < fields.length; f++) 
            {
                ...
                CountsPerYearAndQuarter[year][quarter][field]['counts'] = CountsPerYearAndQuarter[year][quarter][field]['counts']+ 
                                                                            patientGroups[i].data[j]['labelCounts'][field]
		          if  (!CountsPerYearAndQuarter[year][quarter][field]['patients'].includes(patient)) {
    ...
    for (var key1 in CountsPerYearAndQuarter[key]) 
    {
       //iterate through the Labels 
	    ...
	    newEntry['year']=key; // Add Year
         ...
         for (w=0; w<CountsPerYearAndQuarter[key][key1][key2].patients.length; w++)   
         {
