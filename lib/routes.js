...
const routes = [
    ...
    
    {
        ...
        handler: function (request) {
            let answer = request.params.data.split(" ");
           // console.log('***************8', request.payload.output)
            console.log('Task Answer:',answer)

            //reply('Hello, this is the clicked button');
            console.log('Task End')
            return `Answer ${request.params.output}!`;
           //return request.payload.output;
        }
    },
    {
    method: 'POST',
        ...
        handler: function (request) {
           // let answer = request.params.output
            console.log('Task Start ')
           return `Task ${request.params.output}!`;
    ...
    //// Label URI route called the Java API Saja Al
    {
        method: 'GET',
        path:'/labels/{patientIds}', 
        handler: function (request, h) {
            let patientIds = request.params.patientIds.split('+');
            const session = neo4jDriver.session(neo4j.session.READ);
            let promise = session.run(neo4jFunctions.getLabelSummary(patientIds))
                .then(function(neo4jResult) {
                    session.close();
                    ...
                    let neo4jRawArr = neo4jResult.records[0].get('labels'); 
                    //console.log(neo4jRawArr)
                    // Convert the body into desired json data structure
                  // return(neo4jRawArr);

                    return dataProcessor.getLabelSummary(patientIds, neo4jRawArr);
                })
                .catch(function(error) {
                	console.log('Failed to call the neo4j function: getLabelSummary()');
                    console.error(error);
                });
    ...
    {
        method: 'GET',
        path: '/' + apiBaseUriPath + '/allPatients', 
        ...
        options: {
	        ...
	        notes: 'Returns the JSON data of all patients',
        ...
        handler: function (request, h) {
            const session = neo4jDriver.session(neo4j.session.READ);
            let promise = session.run(neo4jFunctions.getAllPatients())
                .then(function(neo4jResult) {
                    session.close();

                    // 'allPatients' is the key
                    // Neo4j function deepphe.getAllPatients() returns a list of all the patients
                    let neo4jRawArr = neo4jResult.records[0].get('allPatients');

                    // Convert the body into desired json data structure
                    return dataProcessor.getAllPatients(neo4jRawArr);
                })
                .catch(function(error) {
                	console.log('Failed to call the neo4j function: getAllPatients()');
                    console.error(error);
                });
    ...
    // Cohort data of all patients from all stages URI route
    {
        method: 'GET',
...
/*
    ...
    {
        method: 'GET',
        path: '/' + apiBaseUriPath + '/terms/{patientIds}', 
        // For API documentation with Swagger
        options: {
	        description: 'Terms of patients', 
	        notes: 'Returns the terms information in JSON for a list of given patient IDs (seperated using the plus sign "+")',
	        tags: ['api'],
	        validate: {
	            params: {
	                patientIds : Joi.string()
	                        .required()
	                        .description('A string of patient IDs, seperated by plus sign')
	            }
	        }
	    },
        handler: function (request, h) {
            let patientIds = request.params.patientIds.split('+');
            const session = neo4jDriver.session(neo4j.session.READ);
            let promise = session.run(neo4jFunctions.getTermsLabelSummary(patientIds))
                .then(function(neo4jResult) {
                    session.close();

                    let neo4jRawArr = neo4jResult.records[0].get('terms'); 

                    // Convert the body into desired json data structure
                    return dataProcessor.getTermsLabelSummary(patientIds, neo4jRawArr);
                })
                .catch(function(error) {
                	console.log('Failed to call the neo4j function: getTermsLabelSummary()');
                    console.error(error);
                });
...
{
    method: 'GET',
    path: '/' + apiBaseUriPath + '/labels/{patientIds}', 
    // For API documentation with Swagger
    options: {
        description: 'labels of terms in the document', 
        notes: 'Returns the terms information in JSON for a list of given patient IDs (seperated using the plus sign "+")',
        tags: ['api'],
        validate: {
            params: {
...
/*
    ...
    {
        method: 'GET',
        path: '/' + apiBaseUriPath + '/documents/{patientIds}', 
        // For API documentation with Swagger
        options: {
	        description: 'Documents of patients', 
	        notes: 'Returns the document information in JSON for a list of given patient IDs (seperated using the plus sign "+")',
	        tags: ['api'],
	        validate: {
	            params: {
	                patientIds : Joi.string()
        ...
        handler: function (request, h) {
            let patientIds = request.params.patientIds.split('+');
            const session = neo4jDriver.session(neo4j.session.READ);
            let promise = session.run(neo4jFunctions.getDocTermSummary(patientIds))
                .then(function(neo4jResult) {
                    session.close();

                    let neo4jRawArr = neo4jResult.records[0].get('documents'); 

                    // Convert the body into desired json data structure
                    return dataProcessor.getDocTermSummary(patientIds, neo4jRawArr);
                })
                .catch(function(error) {
                	console.log('Failed to call the neo4j function: getDocTermSummary()');
                    console.error(error);
                });
    ...
    */
    ...
    // Cancer and tumor summary
    {
        method: 'GET',
        path: '/' + apiBaseUriPath + '/patient/{patientId}/cancerAndTumorSummary', 
        // For API documentation with Swagger
        options: {
            description: 'Patient cancer and tumor summary', 
            notes: 'Returns the cancer and tumor summary information in JSON for a given patient ID',
            tags: ['api'],
            validate: {
                params: {
                    patientId : Joi.string()
