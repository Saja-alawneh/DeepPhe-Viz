...
const neo4jFunctions = {
	...
	/*// The neo4j function deepphe.getTermsLabelSummary() returns a list of terms per patient
	getTermsLabelSummary: function(patientIds) {
		return "return deepphe.getTermsLabelSummary(['"+ patientIds.join("','") + "']) AS terms";

	},*/
	// The neo4j function deepphe.getLabelSummary() returns a list of labels for each term in each document per patient
	getLabelSummary: function(patientIds) {
		return "return deepphe.getLabelSummary(['"+ patientIds.join("','") + "']) AS labels";
	},
	/*// The neo4j function deepphe.getDocTermSummary() returns a list of documents per patient
	getDocTermSummary: function(patientIds) {
		return "return deepphe.getDocTermSummary(['"+ patientIds.join("','") + "']) AS documents";
	},*/
